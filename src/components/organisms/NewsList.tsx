...
const NewsList: VFC<Props> = ({ info }) => {
  return (
    <section className="py-6 mb-16 bg-base-200">
        ...
        {info.map((info) => (
          <SwiperSlide key={info.id}>
            <div className="flex flex-col content-center w-10/12 p-4 border-r border-b-2 border-l mt-8 mx-auto bg-base-100 shadow-md rounded-lg indicator">
              ...
              <h3 className="max-h-16 mb-4 text-lg tracking-tighter overflow-hidden">{info.title}</h3>
              <Link href={`/info/${info.id}`}>
                <a className="btn btn-outline btn-accent w-2/5 min-h-0 h-8 px-0 mr-0 ml-auto text-xs">lead more</a>
              </Link>
            </div>
          </SwiperSlide>
        ))}
